...
# Must be set before any transformers / huggingface_hub / sentence_transformers
...
def _local(folder_name: str) -> str:
    """Return the absolute path string to a downloaded model folder."""
...
st.caption(
    "KB-authoritative mode: KB-grounded high-risk content → PASSED_WITH_WARNING "
    "| Prompt/system attacks still BLOCKED"
)
...
with st.sidebar:
    ...
    strict_mode = st.toggle(
        "Strict mode", value=False,
        help="Blocks safe but out-of-KB questions.",
    )
    kb_authoritative_mode = st.toggle(
        "KB-authoritative mode",
        value=True,
        help=(
            "If ON, high-risk questions grounded in the uploaded KB pass with "
            "warning. Prompt injection and technical attacks still block."
        ),
    )
    enable_hf = st.toggle(
        "Enable Hugging Face safety models", value=False,
        help="Uses locally downloaded models. OFF uses fallback rules only.",
    )
    enable_audit_log = st.toggle(
        "Enable audit log", value=True,
...
# ---------------------------------------------------------------------------
# Knowledge base upload / paste
# ---------------------------------------------------------------------------
st.markdown("### 1. Upload or paste knowledge base")
uploaded = st.file_uploader("Upload KB document", type=["txt", "pdf", "docx"])
default_kb = (
    Path("samples/cybersecurity_kb.txt").read_text(encoding="utf-8")
    if Path("samples/cybersecurity_kb.txt").exists()
    else ""
)
...
# ---------------------------------------------------------------------------
# Build GuardrailSettings and cached filter
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Question input and guardrail run
# ---------------------------------------------------------------------------
